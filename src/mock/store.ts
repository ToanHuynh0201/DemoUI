...
const buildScopeRows = (
	releaseId: ID,
	draft: {
		scopeType: ReleaseDraft["scopeType"];
		scopeOrgIds?: ID[];
		topicId: ID | null;
	},
...
export const useStore = create<Store>((set, get) => {
	...
	const actorUser = () => {
		const id = actor();
		return id
			? (get().db.users.find((user) => user.id === id) ?? null)
			: null;
	};
	...
	const findQuestion = (id: ID) =>
		get().db.questions.find((question) => question.id === id);
	const findAnswer = (id: ID) =>
		get().db.answers.find((answer) => answer.id === id);
	const journalistUserId = (profileId: ID) =>
		get().db.journalist_profiles.find((profile) => profile.id === profileId)
			?.user_id ?? "";
	...
	const approverOf = (orgId?: ID | null) =>
		get().db.users.find(
			(user) => user.org_id === orgId && user.role === "APPROVER",
		)?.id ?? null;
	...
	return {
		...
		createRelease: (draft) => {
			...
			db.press_releases.unshift({
				id,
				release_code: nextCode(
					"TC-2026/",
					db.press_releases.map((item) => item.release_code),
				),
				title: draft.title,
			...
			buildScopeRows(id, draft).forEach((row) =>
				db.release_scopes.push({ id: nid("rs"), ...row }),
			);
			audit({
				action: "Tạo bản thảo thông tin nguồn",
		...
		updateRelease: (releaseId, patch) => {
			...
			if (patch.securityLevel !== undefined)
				release.security_level = patch.securityLevel;
			if (patch.scopeType !== undefined) {
				db.release_scopes = db.release_scopes.filter(
		...
		submitRelease: (releaseId) => {
			...
			release.updated_at = now();
			audit({
				action: "Trình duyệt thông tin nguồn",
				targetType: "PRESS_RELEASE",
				targetId: releaseId,
			});
		...
		decideRelease: (releaseId, action, note) => {
			...
			if (action === "APPROVE") {
				...
				audit({
					action: "Phê duyệt và phát hành thông tin nguồn",
					targetType: "PRESS_RELEASE",
					targetId: releaseId,
				});

				const scopes = db.release_scopes.filter(
					(item) => item.release_id === releaseId,
				);
						...
						scopes.some(
							(scope) =>
								...
								(scope.scope_type === "ORGANIZATION" &&
									scope.org_id === profile.press_agency_id) ||
								(scope.scope_type === "TOPIC" &&
									db.journalist_topics.some(
										(link) =>
											link.profile_id === profile.id &&
											link.topic_id === scope.topic_id,
									)) ||
								(scope.scope_type === "JOURNALIST" &&
									scope.journalist_profile_id === profile.id),
						),
					);
				...
				release.status =
					action === "RETURN" ? "NEEDS_REVISION" : "DRAFT";
				audit({
					action:
						action === "RETURN"
							? "Trả lại thông cáo để chỉnh sửa"
							: "Từ chối thông cáo",
					targetType: "PRESS_RELEASE",
					targetId: releaseId,
					reason: note,
		...
		withdrawRelease: (releaseId, reason) => {
			...
			release.updated_at = now();
			audit({
				action: "Thu hồi thông cáo đã phát hành",
				targetType: "PRESS_RELEASE",
				targetId: releaseId,
				reason,
			});
		...
		createCorrection: (originalId, content) => {
			const { db } = get();
			const original = db.press_releases.find(
			...
			audit({
				action: "Tạo bản đính chính thông cáo",
				targetType: "PRESS_RELEASE",
				targetId: id,
			});
			touch();
		...
		submitQuestion: (draft) => {
			const { db } = get();
			const user = actorUser();
			const profile = db.journalist_profiles.find(
			...
			db.questions.unshift({
				...
				question_code: nextCode(
					"CH-2026/",
					db.questions.map((item) => item.question_code),
				),
			...
			audit({
				action: "Gửi câu hỏi tới cơ quan phát ngôn",
				targetType: "QUESTION",
				targetId: id,
			});
			db.users
		...
		routeQuestion: (questionId, targetOrgId, dueAt, reason) => {
			...
			moveQuestion(
				question,
				"ROUTED",
				reason || `Chuyển ${orgName(targetOrgId)}.`,
			);
			...
			if (approver) {
				notify({
					...
					body: question.title,
					severity:
						question.priority === "URGENT" ? "HIGH" : "MEDIUM",
					targetType: "QUESTION",
					targetId: questionId,
				});
			}
		...
		rejectQuestion: (questionId, reason, duplicateOfId) => {
			...
			if (duplicateOfId)
				question.duplicate_of_question_id = duplicateOfId;
			moveQuestion(question, "REJECTED", reason);
			audit({
				action: "Từ chối tiếp nhận câu hỏi",
				targetType: "QUESTION",
				targetId: questionId,
				reason,
			});
		...
		assignQuestion: (questionId, assigneeId, dueAt, reason) => {
			...
			audit({
				action: previous
					...
					: "Phân công cán bộ xử lý câu hỏi",
				targetType: "QUESTION",
				targetId: questionId,
				reason,
			});
		...
		saveAnswerDraft: (questionId, content) => {
			...
			const question = findQuestion(questionId);
			if (!question) return "";
			const versions = db.answers.filter(
		...
		submitAnswer: (answerId) => {
			...
			if (question)
				moveQuestion(
					question,
					"PENDING_APPROVAL",
					"Trình lãnh đạo duyệt bản trả lời.",
				);
			audit({
				action: "Trình duyệt bản trả lời",
				targetType: "ANSWER",
				targetId: answerId,
			});
			...
			if (action === "APPROVE") {
				...
				if (question) {
					...
					moveQuestion(
						question,
						"ANSWERED",
						"Duyệt và gửi phản hồi chính thức tới phóng viên.",
					);
					...
					notify({
						recipientId: journalistUserId(
							question.journalist_profile_id,
						),
				...
				audit({
					action: "Duyệt và gửi phản hồi chính thức",
					targetType: "ANSWER",
					targetId: answerId,
				});
			} else {
		...
		requestClarification: (questionId, content) => {
			...
			moveQuestion(
				...
				"Đề nghị phóng viên làm rõ nội dung câu hỏi.",
			);
			audit({
				action: "Yêu cầu làm rõ câu hỏi",
				targetType: "QUESTION",
				targetId: questionId,
			});
			notify({
		...
		respondClarification: (clarificationId, content) => {
			const { db } = get();
			const request = db.clarification_requests.find(
			...
			if (question && question.status === "AWAITING_CLARIFICATION") {
				moveQuestion(
					question,
					"IN_PROGRESS",
					"Phóng viên đã phản hồi nội dung làm rõ.",
				);
			}
			audit({
				action: "Phản hồi yêu cầu làm rõ",
				targetType: "QUESTION",
		...
		requestExtension: (questionId, proposedDueAt, reason) => {
			...
			audit({
				action: "Gửi yêu cầu gia hạn xử lý câu hỏi",
				targetType: "QUESTION",
				targetId: questionId,
				reason,
			});
		...
		decideExtension: (extensionId, approve, note) => {
			const { db } = get();
			const request = db.extension_requests.find(
			...
			if (question && approve) {
				...
				if (question.status === "OVERDUE")
					moveQuestion(
						question,
						"IN_PROGRESS",
						"Đã gia hạn, tiếp tục xử lý.",
					);
			}
			audit({
				action: approve
		...
		createEvent: (draft) => {
			...
			const event: EventItem = {
				...
				event_code: nextCode(
					"SK-2026/",
					db.events.map((item) => item.event_code),
				),
		...
		sendInvitations: (eventId, profileIds) => {
			...
			profileIds.forEach((profileId) => {
				const existing = db.invitations.find(
					(item) =>
						...
						item.journalist_profile_id === profileId,
				);
				if (existing) return;
				const profile = db.journalist_profiles.find(
			...
			audit({
				action: `Gửi giấy mời điện tử tới ${profileIds.length} phóng viên`,
				targetType: "EVENT",
				targetId: eventId,
			});
			touch();
		...
		respondInvitation: (invitationId, accept) => {
			...
			const event = db.events.find(
				(item) => item.id === invitation.event_id,
			);
			if (accept && event && invitation.journalist_profile_id) {
				const existing = db.press_badges.find(
				...
				if (!existing) {
					...
					notify({
						recipientId: journalistUserId(
							invitation.journalist_profile_id,
						),
						type: "BADGE_ISSUED",
		...
		cancelEvent: (eventId, reason) => {
			...
			db.press_badges
				.filter((item) => item.event_id === eventId)
				.forEach((badge) => (badge.status = "REVOKED"));
			audit({
				action: "Hủy sự kiện",
				targetType: "EVENT",
				targetId: eventId,
		...
		checkInByCode: (eventId, qrCode, device) => {
			...
			const badge = db.press_badges.find(
				(item) => item.qr_code.toUpperCase() === code,
			);
			...
			if (!badge) {
				const message =
					"Mã QR không đọc được hoặc không tồn tại trong hệ thống.";
				record("ERROR", message);
				return { result: "ERROR", message };
			}
			if (badge.event_id !== eventId) {
				const message =
					"Thẻ được cấp cho sự kiện khác, không hợp lệ tại cổng này.";
			...
			if (
				...
				new Date(badge.valid_to) < new Date()
			) {
				badge.status = "EXPIRED";
			...
			const profile = db.journalist_profiles.find(
				(item) => item.id === badge.journalist_profile_id,
			);
			if (profile && profile.status !== "APPROVED") {
				const message =
					"Hồ sơ phóng viên không còn hiệu lực trên hệ thống.";
				record("DENIED", message, badge.id);
				return { result: "DENIED", message, badgeId: badge.id };
			}
		...
		sendPostEventPackage: (eventId) => {
			...
			audit({
				action: "Gửi tài liệu sau sự kiện",
				targetType: "EVENT",
				targetId: eventId,
			});
			touch();
		...
		handleInterviewRequest: (requestId, status, note, assigneeId) => {
			const { db } = get();
			const request = db.interview_requests.find(
		...
		markNotificationRead: (notificationId) => {
			const notification = get().db.notifications.find(
		...
		markAllNotificationsRead: () => {
			...
			get()
				.db.notifications.filter(
					(item) => item.recipient_id === userId && !item.read_at,
				)
