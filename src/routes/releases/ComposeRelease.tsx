...
export function ComposeRelease() {
  ...
  const ready = title.trim().length > 8 && content.trim().length > 40
  const pressAgencies = db.organizations.filter((org) => org.org_type === 'PRESS_AGENCY')
  ...
  const saveDraft = () => {
    const id = createRelease({
      ...
      securityLevel: level,
      scopeType,
      scopeOrgId: scopeType === 'ORGANIZATION' ? scopeOrgId || null : null,
    })
    return id
  }
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Field label="Tiêu đề">
            <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Tiêu đề thông cáo" />
          </Field>
          <Field label="Tóm tắt" hint="Hiển thị đầu thông cáo, giúp phóng viên nắm nhanh nội dung.">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Mức bảo mật">
              <Select value={level} onValueChange={(value) => setLevel(value as SecurityLevel)}>
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(Object.keys(securityLevel) as SecurityLevel[]).map((key) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Phạm vi phát hành</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <RadioGroup value={scopeType} onValueChange={(value) => setScopeType(value as SimpleScope)}>
            <label className="flex items-center gap-2 text-sm">
              <RadioGroupItem value="ALL" /> Toàn bộ cơ quan báo chí đã đăng ký
            </label>
            <label className="flex items-center gap-2 text-sm">
              <RadioGroupItem value="ORGANIZATION" /> Một cơ quan báo chí cụ thể
            </label>
          </RadioGroup>
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => navigate('/thong-cao')}>
          Hủy
        </Button>
        <Button
          variant="outline"
