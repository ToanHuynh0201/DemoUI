...
export function MyQuestions() {
  ...
  const rows: Row[] = useMemo(() => {
    if (!profile) return []
    return db.questions
      .filter((question) => question.journalist_profile_id === profile.id)
      .map((question) => ({
        id: question.id,
        code: question.question_code,
        title: question.title,
        handlingOrg: question.handling_org_id ? orgName(db, question.handling_org_id) : 'Đang điều phối',
        status: question.status,
  ...
  const columns: ColumnDef<Row, unknown>[] = [
    { accessorKey: 'code', header: 'Mã câu hỏi', cell: ({ row }) => <DocCode>{row.original.code}</DocCode> },
    {
      accessorKey: 'title',
      header: 'Câu hỏi',
      cell: ({ row }) => <p className="line-clamp-2 max-w-lg font-medium">{row.original.title}</p>,
    },
    ...
    },
    {
      ...
      header: 'Ngày gửi',
      cell: ({ row }) => <span className="font-mono text-xs tabular">{formatDate(row.original.submittedAt)}</span>,
    },
    {
      accessorKey: 'dueAt',
      header: 'Hạn trả lời',
      cell: ({ row }) => {
        if (row.original.answeredAt) {
          return (
            <ToneText tone="good">
              <span className="text-xs">Đã trả lời {formatDate(row.original.answeredAt)}</span>
            </ToneText>
          )
        }
        const info = deadlineInfo(row.original.dueAt)
        if (!info) return <span className="text-muted-foreground text-xs">Chưa ấn định</span>
        return (
        ...
        facets={[
          {
            ...
            label: 'Trạng thái',
            options: Array.from(new Set(rows.map((row) => row.statusLabel))).map((value) => ({ value, label: value })),
          },
        ]}
