...
export function QuestionDetail() {
	...
	const answers = useMemo(
		() => (question ? answersOf(db, question.id) : []),
		[db, question],
	);
	const editableAnswer = answers.find(
		(item) => item.status === "DRAFT" || item.status === "NEEDS_REVISION",
	);
	const pendingAnswer = answers.find(
	...
	const clarifications = db.clarification_requests.filter(
		(item) => item.question_id === question.id,
	);
	const extensions = db.extension_requests.filter(
		(item) => item.question_id === question.id,
	);
	const isClosed = ["ANSWERED", "CANCELLED", "REJECTED"].includes(
		question.status,
	);
	const pendingExtension = isClosed
		? undefined
		: extensions.find((item) => item.status === "PENDING_APPROVAL");
	const routings = db.question_routings.filter(
		(item) => item.question_id === question.id,
	);
	const assignments = db.question_assignments.filter(
		(item) => item.question_id === question.id,
	);
	const duplicateOf = db.questions.find(
		(item) => item.id === question.duplicate_of_question_id,
	);
	...
	const orgOptions = db.organizations.filter(
		(org) =>
			org.org_type === "SPOKESPERSON_AGENCY" ||
			org.org_type === "GOVERNMENT_DEPARTMENT" ||
			org.org_type === "DEPT_CULTURE_SPORTS_TOURISM",
	);
	const staffOptions = db.users.filter(
		(item) =>
			item.org_id === question.handling_org_id &&
			(item.role === "STAFF" ||
				item.role === "ADMIN" ||
				...
				<div className="space-y-5">
					<Card>
						...
						<CardContent className="space-y-4">
							<p className="text-sm leading-6 whitespace-pre-line text-pretty">
								{question.content}
							</p>
							...
							{duplicateOf && (
								<div className="rounded border border-[#e4c68a] bg-[#fdf3e2] p-3 text-sm text-[#8a4f06]">
									Câu hỏi này được đánh dấu trùng với{" "}
									<DocCode>
										{duplicateOf.question_code}
									</DocCode>{" "}
									— {duplicateOf.title}
								</div>
							)}
					...
					{!isAwaitingClarification && canRoute && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
									Điều phối câu hỏi
							...
							<CardContent className="space-y-4">
								...
								<div className="flex flex-wrap gap-2">
									<ActionDialog
										...
										title={
											question.handling_org_id
												? "Định tuyến lại câu hỏi"
												: "Chuyển câu hỏi tới cơ quan phát ngôn"
										}
										...
										onConfirm={() => {
											if (!routeOrg || !routeDue) {
												toast.error(
													"Chọn đơn vị xử lý và hạn trả lời trước khi chuyển.",
												);
												return false;
											}
											store.routeQuestion(
												question.id,
												routeOrg,
												fromLocalInput(routeDue),
												routeReason,
											);
											toast.success(
												`Đã chuyển câu hỏi tới ${orgName(db, routeOrg)}.`,
											);
										}}>
										<Field label="Đơn vị xử lý">
											<Select
												value={routeOrg}
												onValueChange={setRouteOrg}>
										...
										<Field
											label="Hạn trả lời"
											hint="Câu hỏi ưu tiên Khẩn phải trả lời trong 24 giờ theo quy chế.">
											<Input
												type="datetime-local"
												value={routeDue}
												onChange={(event) =>
									...
									<ActionDialog
										...
										onConfirm={() => {
											...
											toast.success(
												"Đã từ chối tiếp nhận và thông báo tới phóng viên.",
											);
										}}>
					...
					{!isAwaitingClarification && canAssign && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
									Phân công xử lý
							...
							<CardContent className="space-y-3">
								<p className="text-muted-foreground text-sm">
									...
									<span className="text-foreground font-medium">
										{question.assignee_id
											? userName(db, question.assignee_id)
											: "chưa phân công"}
									</span>
								</p>
								<ActionDialog
									...
									onConfirm={() => {
										if (!assignee || !assignDue) {
											toast.error(
												"Chọn cán bộ và hạn xử lý.",
											);
											return false;
										}
										store.assignQuestion(
											question.id,
											assignee,
											fromLocalInput(assignDue),
											assignReason,
										);
										toast.success(
											`Đã phân công ${userName(db, assignee)}.`,
										);
									}}>
									...
									<Field label="Hạn xử lý">
										<Input
											...
											value={assignDue}
											onChange={(event) =>
												setAssignDue(event.target.value)
											}
										/>
									</Field>
					...
					{!isAwaitingClarification && canDraft && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
							...
							<CardContent className="space-y-4">
								{editableAnswer?.status ===
									"NEEDS_REVISION" && (
									<div className="rounded border border-[#e4c68a] bg-[#fdf3e2] p-3 text-sm text-[#8a4f06]">
										Bản trả lời đã bị trả lại để chỉnh sửa.
										Xem ghi chú của lãnh đạo trong dòng thời
										gian bên phải.
									</div>
								)}
								<AiPanel
									...
									onAccept={(result) => {
										...
										toast.success(
											"Đã đưa bản nháp của AI vào ô soạn thảo. Kiểm tra lại trước khi trình duyệt.",
										);
									}}
								/>
								<Textarea
								...
								<div className="flex flex-wrap gap-2">
									...
									<Button
										onClick={() => {
											...
											toast.success(
												"Đã trình lãnh đạo duyệt bản trả lời.",
											);
										}}>
									...
									<ActionDialog
										...
										onConfirm={() => {
											...
											toast.success(
												"Đã gửi yêu cầu làm rõ tới phóng viên.",
											);
										}}>
									...
									<ActionDialog
										...
										onConfirm={() => {
											if (
												...
												toast.error(
													"Nhập hạn đề xuất và lý do gia hạn.",
												);
												return false;
											}
											store.requestExtension(
												question.id,
												fromLocalInput(extensionDue),
												extensionReason,
											);
											toast.success(
					...
					{!isAwaitingClarification && canApprove && pendingAnswer && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
									Duyệt bản trả lời
							...
							<CardContent className="space-y-4">
								<div className="bg-muted/40 rounded border p-3">
									<p className="text-muted-foreground mb-2 text-xs">
										Phiên bản {pendingAnswer.version} · soạn
										bởi{" "}
										{userName(
											db,
											pendingAnswer.drafted_by_id,
										)}{" "}
										·{" "}
										{formatDateTime(
											pendingAnswer.submitted_at,
										)}
									</p>
									<p className="text-sm leading-6 whitespace-pre-line text-pretty">
								...
								<AiPanel
									kind="check_sensitive"
									inputId={pendingAnswer.id}
									autoRun
								/>
								...
								<div className="flex flex-wrap gap-2">
									<ActionDialog
										...
										onConfirm={() => {
											store.decideAnswer(
												pendingAnswer.id,
												"APPROVE",
												note || "Đồng ý phát hành.",
											);
											toast.success(
												"Đã duyệt, ký số và gửi phản hồi tới phóng viên.",
											);
										}}>
										<Field
											label="Ghi chú phê duyệt"
											hint="Ghi chú được lưu vào hồ sơ xử lý câu hỏi.">
											<Textarea
												value={note}
												onChange={(event) =>
													setNote(event.target.value)
									...
									<ActionDialog
										...
										onConfirm={() => {
											...
											toast.success(
												"Đã trả lại bản trả lời cho cán bộ soạn thảo.",
											);
										}}>
					...
					{!isAwaitingClarification && canApprove && pendingExtension && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
									Đề nghị gia hạn đang chờ quyết định
								</CardTitle>
							</CardHeader>
							<CardContent className="space-y-3">
								<div className="grid gap-3 sm:grid-cols-3">
									<MetaItem label="Người đề nghị">
										{userName(
											db,
											pendingExtension.requested_by_id,
										)}
									</MetaItem>
									<MetaItem label="Hạn hiện tại">
										{formatDateTime(
											pendingExtension.current_due_at,
										)}
									</MetaItem>
									<MetaItem label="Hạn đề xuất">
										{formatDateTime(
											pendingExtension.proposed_due_at,
										)}
									</MetaItem>
								</div>
								<p className="text-sm">
								...
								<div className="flex flex-wrap gap-2">
									<ActionDialog
										...
										onOpen={() => setNote("")}
										onConfirm={() => {
											store.decideExtension(
												pendingExtension.id,
												true,
												note || "Đồng ý gia hạn.",
											);
											toast.success(
												"Đã duyệt gia hạn và cập nhật hạn xử lý.",
											);
										}}>
						...
						<>
							{clarifications
								...
								.map((item) => (
									<Card
										key={item.id}
										className="border-[#e4c68a] bg-[#fffcf5]">
										<CardHeader>
											<CardTitle className="text-base">
												Cơ quan xử lý đề nghị bạn làm rõ
											</CardTitle>
										</CardHeader>
										<CardContent className="space-y-3">
											...
											<Button
												onClick={() => {
													...
													store.respondClarification(
														item.id,
														clarifyResponse,
													);
													setClarifyResponse("");
													toast.success(
														"Đã gửi nội dung làm rõ tới cơ quan xử lý.",
													);
												}}>
												Gửi nội dung làm rõ
											</Button>
								...
								<ActionDialog
									...
									onConfirm={() => {
										store.cancelQuestion(
											question.id,
											note ||
												"Phóng viên chủ động rút câu hỏi.",
										);
					...
					{!isAwaitingClarification && sentAnswer && (
						<Card className="border-[#a8d6bd]">
							<CardHeader className="flex-row items-center justify-between gap-2">
								...
								<StatusBadge
									meta={answerStatus[sentAnswer.status]}
								/>
							</CardHeader>
							<CardContent className="space-y-3">
								<p className="text-sm leading-6 whitespace-pre-line text-pretty">
								...
								<div className="text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1 border-t pt-3 text-xs">
									<span>
										...
										{userName(
											db,
											sentAnswer.approved_by_id,
										)}
									</span>
									<span className="font-mono tabular">
										{formatDateTime(sentAnswer.sent_at)}
									</span>
									{sentAnswer.is_digitally_signed && (
										<span className="flex items-center gap-1">
											...
											<span className="font-mono">
												{
													sentAnswer.signature_transaction_id
												}
											</span>
										</span>
									)}
					...
					{!isAwaitingClarification && answers.length > 1 && (
						<Card>
							<CardHeader>
								<CardTitle className="text-base">
									Các phiên bản bản trả lời
								</CardTitle>
							</CardHeader>
							<CardContent className="space-y-3">
								{answers.map((answer) => (
									<details
										...
										<p className="border-t px-3 py-2 text-sm leading-6 whitespace-pre-line">
											{answer.content}
										</p>
									</details>
								))}
				...
				<div className="space-y-5">
					<Card>
						...
						<CardContent className="grid grid-cols-2 gap-4">
							<MetaItem label="Lĩnh vực">
								{topicName(db, question.topic_id)}
							</MetaItem>
							<MetaItem label="Ưu tiên">
								{priorityLabels[question.priority].label}
							</MetaItem>
							<MetaItem label="Đơn vị xử lý">
							...
							<MetaItem label="Điều phối viên">
								{question.coordinator_id
									? userName(db, question.coordinator_id)
									: "—"}
							</MetaItem>
							...
							<MetaItem label="Hạn phóng viên đề nghị">
								<span className="font-mono text-sm tabular">
									{formatDate(question.requested_deadline)}
								</span>
							</MetaItem>
								...
								{deadline ? (
									<span className="space-y-0.5">
										<span className="block font-mono text-sm tabular">
											{formatDateTime(question.due_at)}
										</span>
							...
							<CardContent className="space-y-3 text-sm">
									...
									<div
										...
										<p className="font-medium">
											{item.is_rerouting
												? "Định tuyến lại"
												: "Định tuyến"}{" "}
											→ {orgName(db, item.target_org_id)}
										</p>
										<p className="text-muted-foreground text-xs">
											{userName(db, item.coordinator_id)}{" "}
								...
								{assignments.map((item) => (
									<div
										...
										className="border-l-2 pl-3">
										<p className="font-medium">
											{item.is_reassignment
												? "Phân công lại"
												: "Phân công"}{" "}
											→ {userName(db, item.assignee_id)}
										</p>
										<p className="text-muted-foreground text-xs">
											Giao bởi{" "}
											{userName(db, item.assigned_by_id)}{" "}
											· hạn {formatDate(item.due_at)}
										</p>
									</div>
								))}
								{extensions.map((item) => (
									<div
										...
										<p className="flex flex-wrap items-center gap-2 font-medium">
											Gia hạn tới{" "}
											{formatDate(item.proposed_due_at)}
											<StatusBadge
												meta={
